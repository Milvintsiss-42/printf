...
/*   Updated: 2022/01/24 17:45:25 by ple-stra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_addpad(t_list **print, int nb)
{
	...
	while (nb--)
		str[nb] = ' ';
	if (!ft_addstr_b(print, str, len))
		return (ft_free(str) - 1);
	free(str);
	return (1);
}

int	ft_addstr_and_padding(t_mod mod, t_list **print, char *str, int len)
{
	int	r;

	if (!ft_checkflag(mod.flags, FLAG_PADR))
		if (!ft_addpad(print, mod.width - len))
			return (-1);
	r = ft_addstr_b(print, str, len);
	free(str);
	if (!r)
		return (-1);
	if (ft_checkflag(mod.flags, FLAG_PADR))
